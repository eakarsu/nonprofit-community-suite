const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4110';
...
const adminCookie = await login('admin@nonprofit-community.local', 'admin123');
const managerCookie = await login('manager@nonprofit-community.local', 'manager123');
const analystCookie = await login('analyst@nonprofit-community.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/programs', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/programs', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/programs/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/programs/approve`, {
  method: 'POST',
  headers: {
